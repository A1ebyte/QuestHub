...
function AficionCrear({
  ...
  const gestionarGenero = (e) => {
    ...
    setForm({ ...form, genre: nuevosGeneros });
  };
  ...
  // Manejo de cantidad de screenshots
  ...
  const enviarFormulario = (e) => {
    ...
    if (validar()) {
      ...
      ServicioVideojuegos.crearEntrada(nuevo)
        .then((response) => {
          enviarNoti(typeToast.SUCCESS, "Videojuego creado correctamente");
          setForm({
            nombre: "",
            description: "",
            genre: [],
          ...
          setVideojuegos([...videojuegos, { ...nuevo, id: response.data.id }]);
          onClose();
        })
        .catch((error) => {
          enviarNoti(typeToast.ERROR, "Error al crear el videojuego");
        });
      return;
    }
